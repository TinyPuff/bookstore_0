...
def go_to_gateway_view(request):
    ...
    try:
        ...
        bank_object = bank_models.banks.Bank.objects.get(
            tracking_code=bank_record.tracking_code
        )
        print(str(bank_object.bank_type))
        bank_object.callback_url = reverse("callback-gateway")
        print(
            f"Info 1: Tracking Code is: {bank_record.tracking_code}, Callback URL is: {bank_object.callback_url}"
        )
...
def callback_gateway_view(request):
    ...
    total_products = len(
        OrderedProductsInfo.objects.filter(
            order=OrderInfo.objects.get(tracking_code=tracking_code)
        )
    )
    ordered_object = OrderInfo.objects.get(tracking_code=tracking_code)
    ordered_object.total_products = total_products
    ordered_object.save()
    request.session["tracking_code"] = bank_record.tracking_code
